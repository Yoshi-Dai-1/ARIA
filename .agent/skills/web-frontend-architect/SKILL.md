...
## 3. 英文書類 (English Documents) および添付文書 (Attachments) の表示戦略

### A. 英文書類 (English Reports)
- **対象**: `english_path` (`raw/edinet/.../english/[doc_id]/`)
- **レンダリング方針**:
  - 英文書類は `EnglishDoc/` フォルダ内の内容（HTML または PDF）が展開されています。
  - フロントエンドはディレクトリ内のファイル一覧をスキャンし、HTML が存在する場合はそれを優先的にインライン表示し、PDF は直接リンクまたはビューアで提供します。
  - **アノマリ対応**: `processed_status` が `english_empty` の場合、パスが `NULL` となっているため、UI 上で「英文書類なし」と明示的に表示してください。

### B. 添付文書 (Attachments)
- **対象**: `attach_path` (`raw/edinet/.../attach/[doc_id]/`)
- **レンダリング方針**:
  - 添付文書（定款、交代届出書等）はバックエンドにより PDF のみが抽出・展開済みです。
  - フロントエンドはディレクトリ内の PDF 一覧を表示し、ユーザーが直接閲覧できるようにします。
  - **アーキテクチャ正解**: ZIP のデコードは不要です。展開済みの静的 PDF への HTTP GET により、低レイテンシなプレビューを実現してください。
