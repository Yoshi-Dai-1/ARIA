...
| カラム名 | 型 | 説明 | ロジック |
| :--- | :--- | :--- | :--- |
| `is_active` | bool | 運用上の追跡対象フラグ。 | システム判断 |
...
## 5. 集約ブリッジ (Code Inheritance)
`ESE140190.csv` に基づき、吸収合併や持株会社化が発生した場合、旧コードの属性を新コードへ自動的に継承します。
- `former_edinet_codes`: カンマ区切りで旧コードを保持し、過去データとのトレーサビリティを確保します。
