...
### 4) Industry & Market Classification (Normalized)
JPXの業種区分など、コード体系は変わらず「名称」のみが変更されるケースに対応するため、定義と実体をハイブリッドに管理します。

**【定義マスタ (`jpx_definitions.parquet`) - SCD Type 2 履歴管理】**
コードと名称のマッピングは `jpx_definitions.parquet` に EAV (Entity-Attribute-Value) 構造で格納され、SCD Type 2 モデルで履歴管理されます。
- 業種名が変更された場合、古い名称のレコードの `valid_to`（適用終了日）が「昨日」に打刻されクローズされます。
- 同時に、新しい名称のレコードが `valid_from`（適用開始日）＝「今日」、`valid_to = None`（現在有効）として追加されます。これにより、過去のいかなる時点の業種名も数学的に再構築可能です。

**【銘柄マスタ (`stocks_master.parquet`) - SCD Type 1 最新状態の保証】**
ウェブアプリからの単一クエリによる高速参照を可能にするため、マスタ上の業種名（`sector_jpx_33` 等）は、常に `jpx_definitions` の「最新の Valid な名称」で上書き（SCD Type 1 同期）されます。これにより、表記揺れや変更の適用漏れを物理的に排除します。
