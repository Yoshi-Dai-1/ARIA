...
description: 日本の証券データ（EDINET）のXBRL/iXBRL解析に関する専門スキル。「予測」を排除し、事実（名前空間・スキーマ参照）に基づく厳格なパースを実現します。US GAAP 等の会計基準によるタグ付けの差異にも対応します。
---
...
## 9. データの不整合ゼロの保証 (Zero Inconsistency Guarantee)
ARIA の分割 worker ジョブによる XBRL 解析では、抽出されたデータは例外なく以下のルールに基づき保存されます。
- **Numerical Facts**: 数値データとしてタグ付けされているものは 100% `financial_values` に保存されます。
- **Qualitative Facts**: 定性情報やテキストブロックとしてタグ付けされているものは 100% `qualitative_text` に保存されます。
- **不整合ゼロ**: 提出された XBRL 内の全ファクトは、必ず上記のいずれか（または両方）に正確に振り分けられ、1件の漏れも生じないことを工学的に保証します。
