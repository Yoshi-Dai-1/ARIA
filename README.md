...
- **Self-healing (Auto-retry)**: 新規取得と同時に、過去60日分の範囲で失敗（failure）または未完了（pending）の書類を自動で検知し、再実行リストに含めます。これにより、一時的なエラーで取り残された書類も自動的に救済されます。
※2016年2月14日以前のデータについては、APIの制限により取得不可。
