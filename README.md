...
`hourly_backfill.yml` により、毎時15分に自動実行されます。2016年2月15日以降の全データを、14日刻みで「過去から現在へ（Forward）」順に **20並列** で取得します。
- **Zero Drift Architecture**: Discoveryジョブが取得したメタデータをArtifactとして共有し、全Workerが同一のメタデータを使用して解析を行うことで、APIのゆらぎによる不整合（Drift）を完全に排除しています。
- **Status Sync (取下検知)**: 既に成功(success)としてカタログにある書類であっても、最新のAPIメタデータで「取下(withdrawn)」が検知された場合、カタログを遡及的に更新 (`retracted`) し、解析対象から除外します。
- **Self-healing (Auto-retry)**: 新規取得と同時に、過去60日分の範囲で失敗または未完了の書類を自動救済します。
- **Resilient Merger**: 万が一一部のデルタファイルが破損していても、そのファイルのみをスキップして統合を継続する堅牢なマージロジックを備えています。

### 3. ドキュメントカタログ (26カラム構成)

`documents_index.parquet` は以下の高密度なメタデータを保持し、高速なフィルタリングと高度な分析を支えます。
